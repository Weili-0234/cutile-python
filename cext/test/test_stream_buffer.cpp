...
static void run_transaction(StreamBufferPool* pool, CUstream stream, size_t size, size_t reps = 1) {
    for (size_t i = 0; i < reps; ++i) {
        CHECK(g_cuStreamSynchronize(stream) == CUDA_SUCCESS);
        StreamBufferTransaction tx = stream_buffer_transaction_open(pool, stream);
...
static StreamBuffer* find_stream_buffer(StreamBufferPool* pool, CUstream s) {
    ...
    CHECK(g_cuStreamGetId(s, &stream_id) == CUDA_SUCCESS);
...
int main() {
    ...
    Py_Initialize();
    CHECK(cuda_loader_init());
    CHECK(g_cuInit(0) == CUDA_SUCCESS);

    // Patch cuEventQuery() so that we can simulate uncompleted events
    orig_cuEventQuery = g_cuEventQuery;
    g_cuEventQuery = patched_cuEventQuery;

    // Create & activate a CUDA context
    CUdevice dev;
    CHECK(g_cuDeviceGet(&dev, 0) == CUDA_SUCCESS);
    CUcontext ctx;
    CHECK(g_cuDevicePrimaryCtxRetain(&ctx, dev) == CUDA_SUCCESS);
    CHECK(g_cuCtxSetCurrent(ctx) == CUDA_SUCCESS);
    ...
    // Create a stream
    CUstream stream;
    CHECK(g_cuStreamCreate(&stream, 0) == CUDA_SUCCESS);
    ...
    CHECK(g_cuStreamCreate(&other_stream, 0) == CUDA_SUCCESS);
